...
def calculate_feature_for_all_audio_files(args):
    ...
    hf.create_dataset(
        name='feature', 
        shape=(0, frames_num_clip, mel_bins), 
        maxshape=(None, frames_num_clip, mel_bins), 
        dtype=np.float32)
    ...
    for (n, filename) in enumerate(meta_dict['filename']):
        ...
        tag = random.randint(0,frames_num-frames_num_clip-1)
        feature = feature[tag:tag+frames_num_clip]
        hf['feature'].resize((n + 1, frames_num_clip, mel_bins))
