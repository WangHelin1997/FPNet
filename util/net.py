...
class Cnns(nn.Module):
    ...
    def __init__(self,  sub_bins=20, sub_stride=10, mel_bins=40, classes_num=50, activation='logsoftmax'):
        super(Cnns, self).__init__()
        ...
        self.conv1 = nn.ModuleList(
            [nn.Conv2d(in_channels=1, out_channels=32, kernel_size=5, stride=1, padding=2) for _ in
             range(self.sub_num)])
        self.conv1_bn = nn.ModuleList([nn.BatchNorm2d(32) for _ in range(self.sub_num)])

        self.conv2 = nn.ModuleList(
            [nn.Conv2d(in_channels=32, out_channels=32, kernel_size=5, stride=1, padding=2) for _ in
             range(self.sub_num)])
        self.conv2_bn = nn.ModuleList([nn.BatchNorm2d(32) for _ in range(self.sub_num)])
        self.mp2 = nn.ModuleList([nn.MaxPool2d((2, 5)) for _ in range(self.sub_num)])
        
        self.conv3 = nn.ModuleList(
            [nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, stride=1, padding=1) for _ in
             range(self.sub_num)])
        self.conv3_bn = nn.ModuleList([nn.BatchNorm2d(64) for _ in range(self.sub_num)])
 

        self.conv4 = nn.ModuleList(
            [nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1) for _ in
             range(self.sub_num)])
        self.conv4_bn = nn.ModuleList([nn.BatchNorm2d(64) for _ in range(self.sub_num)])
        self.mp4 = nn.ModuleList([nn.MaxPool2d((2, 5)) for _ in range(self.sub_num)])

        
        self.conv5 = nn.ModuleList(
            [nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3, stride=1, padding=1) for _ in
             range(self.sub_num)])
        self.conv5_bn = nn.ModuleList([nn.BatchNorm2d(128) for _ in range(self.sub_num)])

        self.conv6 = nn.ModuleList(
            [nn.Conv2d(in_channels=128, out_channels=128, kernel_size=3, stride=1, padding=1) for _ in
             range(self.sub_num)])
        self.conv6_bn = nn.ModuleList([nn.BatchNorm2d(128) for _ in range(self.sub_num)])
        self.mp6 = nn.ModuleList([nn.MaxPool2d((5, 10)) for _ in range(self.sub_num)])


        self.fc1 = nn.ModuleList([nn.Linear(128, 32) for _ in range(self.sub_num)])
...
#         self.fcGlobal.append(nn.Linear(tempNeurons, 50))
        ...
        self.conv_block1 = ConvBlock_mix(in_channels=1, out_channels=32)
        self.conv_block2 = ConvBlock_mix(in_channels=32, out_channels=64)
        self.conv_block3 = ConvBlock_mix(in_channels=64, out_channels=128)
        self.conv_block4 = ConvBlock_mix(in_channels=128, out_channels=256)
        self.fc2 = nn.Linear(256, 128)
        self.dropout = nn.Dropout(p=0.5)
        self.fc = nn.Linear(tempNeurons+ 128, 128)
        self.dropouth = nn.Dropout(p=0.5)
